...
export const marketService = {
  ...
  async getIntradayData(
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(`Error intraday fetch: ${response.statusText}`);
      ...
      if (Array.isArray(rawContent)) {
        return rawContent.map((item: any) => {
          const timeLabel = item.f
            ? item.f.split(" ")[1]?.substring(0, 5) || item.f
            : "00:00";
          return {
            time: timeLabel,
  ...
  async getMarketTrades(
    ...
    try {
      ...
      // 1. Calcular la fecha objetivo (Último día hábil cerrado)
      // Si es fin de semana o es temprano antes de que consolide el día anterior, viajamos al pasado útil.
      let diasAtras = 0;

      if (diaSemana === 0) { // Domingo -> Pedir Viernes
        diasAtras = 2;
      } else if (diaSemana === 6) { // Sábado -> Pedir Viernes
        diasAtras = 1;
      } else if (diaSemana === 1 && horas < 16) { // Lunes antes del cierre -> Pedir Viernes
        diasAtras = 3;
      } else if (horas < 16) { 
        // Martes a Viernes antes de las 4 PM: El día de hoy sigue activo/liquidando, 
        // pedimos los hechos consolidados de AYER.
        diasAtras = 1;
      } else {
        // Después de las 4 PM en día hábil, la API ya debería tener listo el cierre de hoy.
        diasAtras = 0;
      }

      const targetDateObj = new Date(ahora.getTime() - diasAtras * 24 * 60 * 60 * 1000);
      const formateador = new Intl.DateTimeFormat('fr-CA', { year: 'numeric', month: '2-digit', day: '2-digit' });
      const fechaCalculada = formateador.format(targetDateObj); // Devuelve "YYYY-MM-DD" perfecto

      // 2. Definir rango de horas óptimo y seguro (Bloque fijo de alta liquidez)
      // Como es un endpoint histórico, lo mejor es pedir una hora fija donde siempre hay transacciones garantizadas
      const horaInicio = "11:00";
      const horaFinal = "12:00";

      const targetTicker = encodeURIComponent(emisoraSerie);

      // 3. Construcción estricta de la URL con todos los campos requeridos
      const url = `${BASE_URL}/hechos?token=${token}&emisora_serie=${targetTicker}&fecha=${fechaCalculada}&horai=${horaInicio}&horaf=${horaFinal}`;

      const response = await fetch(url);

      if (!response.ok) {
        console.warn("Parámetros calculados en fallo:", { emisoraSerie, fechaCalculada, horaInicio, horaFinal });
        console.error(`[Fetch Error] Status: ${response.status}, Text: ${response.statusText}`);
        return []; // Retorno preventivo para no tumbar la UI
      }
